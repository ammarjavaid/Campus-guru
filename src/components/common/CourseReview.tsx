...
function CourseReview({ review }: proptypes) {
  const params = useParams();
  const lng: string = params?.lng as string;
  const { t } = useTranslation(lng, "resultDetails");
  ...
  const upVoteHandler = (review: CourseReview) => {
    setLoading(true);
    reviewService
      .upVoteCourseReview(Number(review.id))
      .then((response: any) => {
  ...
  const downVoteHandler = (review: CourseReview) => {
    setLoading(true);
    reviewService
      .downVoteCourseReview(Number(review.id))
      .then((response: any) => {
    ...
    <div className='rounded-[10px] bg-[#F9F9FF] md:py-5 md:px-6 px-4 py-4 mb-2'>
      <div className='md:flex justify-between items-center mb-4'>
        <div className='flex items-center'>
          ...
          <span className='flex items-center bg-[#EAECFA] rounded-[20px] text-[#717EF6] px-4 py-1 ml-3 text-[16px]'>
            <span>{calculateCourseAverageRating(review).toFixed(1)}</span>
            &nbsp;
            <img
      ...
      <div className='grid md:grid-cols-2 grid-cols-1 gap-4 mt-5'>
        <div className='flex col-span-1 flex-col md:flex-row  items-center gap-2  mb-5'>
          <div className='w-full md:w-[20%]'>{t("difficulty")}</div>
          <div className='flex-1 w-full md-w-unset '>
            <RatingLines averageReview={review.difficultyReview} />
          </div>
        </div>
        <div className='flex col-span-1 flex-col md:flex-row  items-center gap-2 mb-5'>
          <div className='w-full md:w-[20%]'>{t("content")}</div>
          <div className='flex-1 w-full md-w-unset '>
            <RatingLines averageReview={review.contentReview} />
          </div>
        </div>
        <div className='flex col-span-1 flex-col md:flex-row  items-center gap-2 mb-5'>
          <div className='w-full md:w-[20%]'>{t("structure")}</div>
          <div className='flex-1 w-full md-w-unset '>
            <RatingLines averageReview={review.structureReview} />
          </div>
        </div>

        <div className='flex col-span-1 flex-col md:flex-row  items-center gap-2 mb-5'>
          <div className='w-full md:w-[20%]'>{t("relevance")}</div>
          <div className='flex-1 w-full md-w-unset '>
            <RatingLines averageReview={review.relevanceReview} />
          </div>
        </div>
      </div>
    </div>
